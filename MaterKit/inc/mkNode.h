...
public:
	...
	mkPDFs getAddedPhasePDFs(const mkReal &omega, const mkReal& phi, \
	...
	/// Lambda*(f-f_eq)
	void collision_MRT0(const mkReal& omega, const mkVector3d &force);

	/// Lambda*(f-f_eq)
	void collision_MRT1(const mkReal& omega, const mkVector3d &force);

	/// MiS*(m_pre-m_equ)
	void collision_MRT2(const mkReal& omega, const mkVector3d &force);

	void collision_MRT1_solute(const mkReal& omega);
	...
	std::vector<mkPDFs> gVec_out;
	std::vector<mkPDFs> gVec_hlp;
	std::vector<mkPDFs> gVec_nbr;

	std::vector<double> phiVec;
	std::vector<double> phiVec_old;
	std::vector<double> d_phiVec;
	...
	mkPDFs x_out;
	mkPDFs x_hlp;
	//std::vector<mkPDFs> xVec_out;
	//std::vector<mkPDFs> xVec_hlp;
